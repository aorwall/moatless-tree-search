...
from moatless.completion.schema import AllMessageValues, ChatCompletionAssistantMessage, ChatCompletionUserMessage, ResponseSchema
...
class ValueFunctionDebateConclusion(ResponseSchema):
...
class MultiAgentDebate(BaseModel):
    ...
    def conduct_debate(
        ...
        system_prompt: str,
        output_format: type[ResponseSchema],
    ):
        if not messages:
    ...
    def construct_debate_message(self, agents, idx):
        ...
        return ChatCompletionUserMessage(role="user", content=prefix_string)
    ...
    def generate_conclusion(
        ...
        **Agent Answers:**
        ...
        conclusion_context = [
            ...
            ChatCompletionUserMessage(role="user", content=conclusion_prompt),
        ]
    ...
    def format_agent_message(self, action_request, completion) -> ChatCompletionUserMessage | None:
        if action_request:
            content = json.dumps(
                {
            ...
            return ChatCompletionUserMessage(role="user", content=content)  # type: ignore
        elif completion.choices and completion.choices[0].message:
            return ChatCompletionUserMessage(role="user", content=completion.choices[0].message.content)  # type: ignore
        else:
            return None
