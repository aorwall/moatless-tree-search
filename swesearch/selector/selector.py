...
from swesearch.selector.similarity import calculate_similarity
from swesearch.utils.parse import parse_value, parse_explanation
...
from swesearch.selector.prompt import SYSTEM_PROMPT, ALL_EXAMPLES


class LLMSelector(Selector):
    type: Literal["LLMSelector"] = "LLMSelector"
    completion: Optional[BaseCompletionModel] = Field(
    ...
    def __init__(
        self, completion: BaseCompletionModel = None, max_iterations: int = None, **kwargs
    ):
    ...
    def build_ascii_tree(
        ...
        messages = [ChatCompletionUserMessage(role="user", content=user_content)]
