...
def collect_fail_reasons(
    trajectories: List[Tuple[SearchTree, str]],
) -> Dict[str, List[tuple[str, Node, SearchTree]]]:
...
def main():
    ...
    with reason_tab:
        if not fail_reasons:
            ...
            fail_reason_options = [
                f"{reason} ({len(nodes)})"
                for reason, nodes in sorted(
                    fail_reasons.items(), key=lambda x: len(x[1]), reverse=True
                )
            ]
            selected_fail_reason = st.selectbox(
                "Select fail reason:", options=fail_reason_options
            )
    ...
    with flag_tab:
        if not flags:
            ...
            flag_options = [
                f"{flag} ({len(nodes)})"
                for flag, nodes in sorted(
                    flags.items(), key=lambda x: len(x[1]), reverse=True
                )
            ]
