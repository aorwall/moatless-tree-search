...
from swesearch.debate import MultiAgentDebate
...
class AgentDiscriminatorChoice(ResponseSchema):
    ID: int
    EXPLANATION: str
...
class AgentDiscriminator(Discriminator):
    ...
    def __init__(self, completion: BaseCompletionModel, n_agents: int, n_rounds: int):
...
<ID>: The ID of the change you believe is the most appropriate"""
        ...
        if debate:
            ...
            completion_response = self.completion.create_completion(
                messages,
                system_prompt=SYSTEM_MESSAGE,
                response_schema=AgentDiscriminatorChoice,
            )
