...
class ValueFunction(BaseModel):
    ...
    correction_award: Optional[int] = Field(0, description="The reward value to automatically assign when the agent expects a correction.")
    ...
    def get_reward(self, node: Node) -> Tuple[Reward, Optional[Completion]]:
        if node.observation.expect_correction and self.correction_award is not None:
            logger.info(f"Expecting a correction, assigning reward {self.correction_award}")
