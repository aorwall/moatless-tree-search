...
class Finish(Action):
    ...
    enforce_patch: bool = Field(default=False, description="Whether to enforce that the file context has a patch")
    enforce_test_patch: bool = Field(default=False, description="Whether to enforce that the file context has a test patch")
    ...
    def execute(self, args: FinishArgs, file_context: FileContext | None = None, workspace: Workspace | None = None):
        if self.enforce_patch and not file_context.has_patch():
            return Observation(message="No files was updated, you cannot finish unless you have made changes to the files", terminal=False)
        if self.enforce_test_patch and not file_context.has_test_patch():
            return Observation(message="No test files was updated, you cannot finish unless you have updated existing tests or added new tests", terminal=False)
