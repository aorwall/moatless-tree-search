...
class FeedbackGenerator(BaseModel, ABC):
    ...
    def model_validate(cls, obj: Any) -> "FeedbackGenerator":
        if isinstance(obj, dict):
            obj = obj.copy()
            feedback_class_path = obj.pop("feedback_class", "moatless.feedback.reward_feedback.RewardFeedbackGenerator")
            ...
            try:
                ...
                logger.warning(f"Failed to load feedback generator class {feedback_class_path}, defaulting to RewardFeedbackGenerator: {e}")
                from moatless.feedback.reward_feedback import RewardFeedbackGenerator
                return RewardFeedbackGenerator(**obj)
